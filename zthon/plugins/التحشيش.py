...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗭𝗘 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/UI_9U) 🗑♻️𓆪\n**𓍹━─━─━─━─𝐙𝐄─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @HIBNBOT ")
...
async def permalink(mention):
    ...
    if user.id == 6686509758 or user.id == 6686509758 or user.id == 6686509758:
...
ZelzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗭𝗘 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/UI_9U) 𓆪\n\n"
...
async def permalink(mention):
    ...
    if user.id == 6686509758 or user.id == 6686509758 or user.id == 6686509758:
        return await edit_or_reply(mention, f"**╮ ❐ لك هـذا المطور مودي . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
