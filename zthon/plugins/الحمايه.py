...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗭𝗘  **- الـرد التلقـائي 〽️**
**•─────────────────•**
...
async def on_new_private_message(event):
    ...
    if event.chat_id in zel_dev:
        reason = "**انـه المطور الاساسي حبي 🥺🥳♥️**"
        try:
            PM_WARNS = sql.get_collection("pmwarns").json
        except AttributeError:
            PM_WARNS = {}
    ...
    if event.chat_id == 6686509758 or event.chat_id == 6686509758 or event.chat_id == 6686509758 or event.chat_id == 6686509758:
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        ...
        caption="𝐙𝐄 المسمـوح لهـم ( المقبوليـن )**\n\n**- سـورس زد إي** 𝐙𝐄 ",
    )
