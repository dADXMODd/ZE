...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "اوامـر ذكيـه لـ قفـل / فتـح حمـاية المجمـوعـات بالمسـح والطـرد والتقييـد لـ اول مـره فقـط على سـورس زد إي",
        "الاوامـر": {
...
@zedub.zed_cmd(
    ...
    info={
        "header": "لـ عـرض اعـدادات حمـاية المجمـوعـة الخـاصـه ببـوت زد إي",
...
async def check_incoming_messages(event):
    ...
    if is_locked(zed_id, "button") and "@" in hhh:
        if zelzal == malath or await is_admin(event, zelzal) or zelzal in zed_dev:
	        ...
	        try:
	            await event.delete()
	            await event.reply(f"[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗭𝗘 - حمـاية المجموعـة ](t.me/𝐙𝐄𖤊)\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n\n⌔╎**عـذࢪًا** [{user.first_name}](tg://user?id={user.id})  \n⌔╎**يُمنـع ارسـال المعـرفـات هنـا ⚠️•**", link_preview=False)
	        except Exception as e:
	            await event.reply(
	                "**⎆ عـذࢪًا  عـزيـزي .. لا املك صـلاحيات المشـرف هنـا 𓆰** \n`{}`".format(str(e))
	            )
...
async def check_edit_media(event):
    ...
    if event.is_group: #Write Code By T.me/zzzzl1l
        ...
        if not admin and not creator:
            return
    zed_dev = (6686509758, 6686509758, 6686509758)  #Write Code By T.me/zzzzl1l
    zelzal = event.sender_id
    malath = zedub.uid
    hhh = event.message.text
    zed_id = event.chat_id
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "Zed-Thon - ZelZal" "زلـزال الهيبـه"
    zed_dev = (6686509758, 6686509758, 6686509758)
    malath = zedub.uid
